...
const LANES = [
  { id: "kick", label: uiText.drumLane.kick, color: "#5ec26b", match: (pitch: number) => pitch <= 36 },
  { id: "snare", label: uiText.drumLane.snare, color: "#f07b7b", match: (pitch: number) => pitch > 36 && pitch <= 40 },
  { id: "hat", label: uiText.drumLane.hat, color: "#7ad7ff", match: (pitch: number) => pitch === 42 || pitch === 46 },
  { id: "fill", label: uiText.drumLane.fill, color: "#e0b341", match: (pitch: number) => pitch === 45 || pitch === 47 || pitch === 39 }
];
...
export function Drummer({ clip }: { clip: Clip }) {
  ...
  return (
    <section className="grid h-full min-h-0 gap-3 bg-graphite-950/60 p-3 lg:grid-cols-[minmax(0,1fr)_260px]">
      <div className="grid min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] rounded-md border border-line bg-black/20">
        <div className="flex min-h-10 items-center justify-between gap-2 border-b border-line px-3 py-2">
          <div className="flex min-w-0 items-center gap-2">
            <Drum size={16} className="shrink-0 text-meter-amber" />
            <span className="truncate text-sm font-black text-ink-high">{settings.preset}</span>
          </div>
          <span className="text-xs font-black text-ink-body">소리 {clip.notes?.length ?? 0}개</span>
        </div>

        <div
          ref={padRef}
          className="relative m-3 min-h-52 overflow-hidden rounded-md border border-line bg-[linear-gradient(to_right,rgba(94,194,107,.16),rgba(224,179,65,.18)),linear-gradient(to_top,rgba(15,18,23,.88),rgba(255,255,255,.08))]"
          onPointerDown={(event) => {
      ...
      <div className="min-h-0 space-y-3 overflow-y-auto rounded-md border border-line bg-white/[0.04] p-3">
        <div className="flex items-center gap-2">
          <SlidersHorizontal size={15} className="text-meter-cyan" />
          <span className="panel-title">{uiText.common.drummer}</span>
        </div>

        <label className="block text-xs font-bold text-ink-body">
          프리셋
          <select
            className="mt-2 h-9 w-full rounded border border-line bg-surface-base px-2 text-sm font-bold text-ink-high outline-none focus-visible:ring-2 focus-visible:ring-ink-accent"
        ...
        <label className="block text-xs font-bold text-ink-body">
          복잡도 {percent(settings.complexity)}
          <input
