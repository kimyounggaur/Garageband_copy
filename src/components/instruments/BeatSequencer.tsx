    ...
    <div className="grid min-h-full gap-3 lg:grid-cols-[minmax(0,1fr)_220px]">
      <div className="min-w-0 rounded-md border border-line bg-black/20 p-3">
        <div className="mb-2 flex items-center justify-between gap-2">
          <span className="panel-title">비트 시퀀서</span>
        ...
        <div className="space-y-1">
          {DRUM_LANES.map((lane) => (
            <div key={lane.id} className="grid grid-cols-[72px_minmax(0,1fr)] items-center gap-2">
              <div className="truncate text-xs font-black uppercase tracking-[0.08em] text-ink-body">{uiText.drumLane[lane.id]}</div>
                    ...
                    <button
                      ...
                      className={`h-7 rounded border text-[10px] font-black transition ${
                        activeVelocity !== undefined
                          ? "border-accent-play bg-accent-play/25 text-ink-high"
                          : isBarStart
                            ? "border-line bg-white/[0.08] text-graphite-500"
                      ...
                      title={`${uiText.drumLane[lane.id]} ${step + 1}단계`}
                      aria-label={`${uiText.drumLane[lane.id]} ${step + 1}단계`}
                    >
